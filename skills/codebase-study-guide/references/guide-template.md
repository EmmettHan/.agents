# Study Guide Content Template

Use this structure as the content schema when writing the guide. The default deliverable is a self-contained HTML file; this Markdown outline is an authoring reference, not the final output format. Adapt section depth to the codebase's complexity, but preserve the ordering — it follows the pedagogical progression from purpose to patterns to practice.
...
Render the completed guide using the HTML contract in [html-output.md](html-output.md). Preserve the section order, code walkthroughs, diagrams, source evidence, self-tests, and exploration tasks. Use semantic HTML and inline CSS so the result can be opened directly without a build step or network access.
