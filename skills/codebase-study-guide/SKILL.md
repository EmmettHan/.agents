...
description: "Generate a pedagogically-grounded HTML study guide for learning an unfamiliar codebase. Use when the user wants to onboard onto a codebase, understand a project's architecture, create learning materials for a team, or asks things like \"help me learn this codebase\", \"create an onboarding guide\", \"I'm new to this project\", \"how does this system work\", \"study guide for this repo\", or \"explain this codebase to me\". Produces a self-contained HTML guide that builds understanding from purpose to systems to patterns, using evidence-based learning techniques (elaborative interrogation, concept mapping, threshold concepts, worked examples, progressive disclosure)."
---

# Codebase Study Guide

Generate a self-contained HTML study guide that builds durable understanding of a codebase using evidence-based learning techniques. The guide helps readers grasp not just what the code does, but why it exists and how its parts relate. Use [references/html-output.md](references/html-output.md) for the output contract.

## Output Defaults

- Use Simplified Chinese by default unless the user requests another language.
- Produce a self-contained HTML file by default; follow an explicitly requested output path without hardcoding any user-specific directory into the skill.
...
Follow the content structure in [references/guide-template.md](references/guide-template.md), then render it as HTML. Do not emit the Markdown template as the default artifact. Read [references/html-output.md](references/html-output.md) before creating the file. Key principles while writing:
...
- Prefer inline HTML/CSS/SVG diagrams so the file works offline.
- Use `graph TD` for system architecture and data flow, `sequenceDiagram` for request flows, and `classDiagram` for domain models only when the renderer is embedded or locally available.
- Never make the guide depend on an external CDN for its core content.
- Keep diagrams focused — one concept per diagram, not everything at once.

**For the HTML presentation:**
- Use a readable light page background and explicit dark backgrounds with light text for code blocks.
- Show absolute source paths and line anchors for evidence; make local links clickable when the environment permits.
- Use semantic headings, a table of contents, responsive layout, and visual callouts for threshold concepts and self-tests.
- Keep the output self-contained: inline CSS, no external fonts, scripts, images, or network resources unless the user explicitly requests them.
...
Write the guide as a single self-contained HTML file placed at a sensible location (`.claude/docs/study-guide.html` or as specified by the user). If the user specifies a different directory or filename, follow that request and choose a descriptive `.html` filename when needed. Markdown may be used as an intermediate drafting format, but it is not the default deliverable. The reader should need only the HTML file and access to the referenced codebase.
